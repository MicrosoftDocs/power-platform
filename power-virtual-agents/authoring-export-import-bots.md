---
title: "Export and import copilots using solutions (contains video)"
description: "Transfer copilots between environments by adding them to Power Apps solutions."
...
You can export and import copilots using [solutions](/power-platform/alm/solution-concepts-alm) so you can move your copilots across multiple [environments](/power-platform/admin/environments-overview).
...
> [!NOTE]
> You can't export [topic-level or node-level comments](authoring-comments.md) when you export a copilot.
...
Move your copilots across [environments](/power-platform/admin/environments-overview) by exporting and importing them using [solutions](/power-platform/alm/solution-concepts-alm). Exporting and importing your copilots can be useful if you use different environments for different purposes or if you employ ring-deployment methodologies. For example, you might have one environment where you internally test and validate copilots, another where you test copilots with a subset of users, and a production environment where you share your copilots with customers.
...
- Removing a copilot from a solution doesn't remove its components from the solution. You should remove the components separately.  
    ...
    >
    > You should only make changes to topics from Microsoft Copilot Studio. Removing or changing a copilot's components from Power Apps will cause the export and import to fail.
...
> [!NOTE]
> You'll need the minimum System Customizer security roles to export and import copilots using solutions. Learn more about [configuring user security to resources in an environment](/power-platform/admin/database-security).
...
3. Select **Go to Power Apps Solutions**.
    ...
    :::image type="content" source="media/authoring-export-import-bots/export-settings-powerapps.png" alt-text="Screenshot of the copilot export prompt.":::
...
> [!NOTE]
> Removing a copilot from a solution doesn't remove its components from a solution. Removal of the components should be done separately.  
...
> [!IMPORTANT]
> If your copilot has a large number of components (for example, more than 250 topics or more than 100 entities), see [Export using the classic experience](/power-apps/maker/data-platform/export-solutions#export-using-the-classic-experience).
...
1. In the **Import a solution** pane, select **Browse**, select the .zip file that contains the solution with your copilot, and then select **Next**.
...
1. If your copilot has [user authentication](configuration-end-user-authentication.md) enabled, you'll need to configure it again.

1. In the **Objects** pane select **Chatbot**. Then select the copilot's name to open the copilot in the Microsoft Copilot Studio portal.

    :::image type="content" source="media/authoring-export-import-bots/select-bot.png" alt-text="Screenshot of a list of copilots and environments in Microsoft Copilot Studio.":::

    You can also open Microsoft Copilot Studio directly and then open your copilot in the environment you imported it to.

> [!IMPORTANT]
>
> - You must [publish your imported copilot](publication-fundamentals-publish-channels.md) before it can be shared.
> - It may take up to 24 hours for your copilot's icon to appear everywhere.

### Add components to a copilot in a custom solution

If you add components, such as topics or flows, to your copilot in Microsoft Copilot Studio, you'll also need to add them to the copilot in your unmanaged solution.

1. Open your solution in the Power Apps portal.

1. Select **Chatbots**, and then select your copilot in the list.
...
In Microsoft Teams, you can export and import copilots between individual teams using the Power Apps Teams app.

You might want to do this to copy a working copilot into a new team so you don't have to build it from scratch.

### Export your copilot in Teams
...
When selecting the customizations to export, select the copilot you want to export. You can use the filter on the side pane to only show copilots within that team.

The copilot will be saved into the **Downloads** folder on your PC.

### Import your copilot in Teams
...
Your imported copilot will now appear in the Microsoft Copilot Studio app in Teams, under the team you imported it into.
