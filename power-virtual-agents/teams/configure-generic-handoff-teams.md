...
## Prerequisites
- You need to have [built a bot using Power Virtual Agents](authoring-first-bot-teams.md)  
...
## Overview
  ...
  ![ILLUSTRATION SHOWING GENERIC ADAPTER DATAFLOW](media/generic-adapter-illustration-teams.png "ILLUSTRATION SHOWING GENERIC ADAPTER DATAFLOW")
...
1. Once the end user [triggers hand-off](advanced-hand-off-teams.md#triggering-hand-off-to-a-live-agent), Power Virtual Agents starts hand-off with full conversational context.
...
While it is outside the scope of this document to cover what a custom adapter could contain, the following sample hand-off message, based on what Power Virtual Agents generates as part of our [standard hand-off to a live agent experience](advanced-hand-off-teams.md), can help get you started. 
...
Hand-off is currently only supported over DirectLine - [learn more about interacting with the bot over DirectLine](publication-connect-bot-to-azure-bot-service-channels-teams.md#sample-code-example). Upon hand-off, an event activity called `handoff.initiate` is raised and sent to the adapter. 
...
To use [conversational context](advanced-hand-off-teams.md#contextual-variables-available-upon-hand-off), you must parse the `handoff.initiate` event activity. 
