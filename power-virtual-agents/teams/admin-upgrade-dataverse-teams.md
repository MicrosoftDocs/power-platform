...
[comparison](/power-platform/admin/about-teams-environment#conceptual-model) between Dataverse for Teams and Dataverse environments). This can be useful, for example, if you are running into Dataverse for Teams
[capacity limits](/power-platform/admin/about-teams-environment#capacity-limits).
...
When an environment is being upgraded, users will not be able to edit or create bots in the environment. The users will be informed about the ongoing upgrade in the Power Virtual Agents for Teams app and blocked
from accessing that environment.
...
During the upgrade, users will experience issues while chatting with your bot, so you should plan for or notify your users of an expected downtime.
...
Upgraded environments are indicated with a diamond icon in the list of environments under the **Chatbots** tab.
...
>[!NOTE]
>To access the Power Virtual Agents web app, users need a [standalone PVA license](https://docs.microsoft.com/en-us/power-virtual-agents/requirements-licensing-subscriptions) or an individual [trial](https://docs.microsoft.com/en-us/power-virtual-agents/sign-up-individual).

After the upgrade, the bot will continue to work as it did before the upgrade without you having to take any action.

If bot authors want to share a bot with other authors after you have upgraded an environment, you will have to make the authors team owners in the team where the environment was created. This will not work if the
**Teams Owner** security group has been removed from the environment in the Power Platform admin center. This bug will be resolved in an upcoming update, and you'll be able to [share bots with co-authors](https://docs.microsoft.com/en-us/power-virtual-agents/admin-share-bots) as you currently can in standard Dataverse environments.
