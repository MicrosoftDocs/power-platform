...
- [Enable end-user authentication with Azure Active Directory](configuration-end-user-authentication-teams.md#use-azure-active-directory-as-your-identity-provider) and [add an authentication topic to your bot](advanced-end-user-authentication-teams.md)
- [Use a custom canvas](customize-default-canvas-teams.md) 
...
The following illustration shows how a user is signed in without seeing a login prompt (SSO) in Power Virtual Agents:
  ...
  ![Illustration of SSO authentication flow](media/sso-illustration-teams.png)

1. The bot user enters a phrase that [triggers a  sign in topic](advanced-end-user-authentication-teams.md). This topic is designed to sign the user in and use the user's [authenticated token (`AuthToken` variable)](advanced-end-user-authentication-teams.md#authtoken-variable).

2. Power Virtual Agents sends a login prompt to allow the user to sign in with their configured identity provider.

3. The bot's [custom canvas](customize-default-canvas-teams.md) intercepts this sign in prompt and requests an on-behalf-of (OBO) token from Azure Active Directory (Azure AD). The canvas sends the token to the bot.
...
This needs to be a separate app registration from the one you [created when you configured authentication with Azure AD](configuration-end-user-authentication-teams.md#use-azure-active-directory-as-your-identity-provider).
...
1. Go to [App registrations](https://portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationsListBlade), either by selecting the icon or searching in the top search bar.
    ...
    ![Screenshot highlighting app registrations tile on the Azure portal](media/sso-app-registrations-teams.png "Screenshot highlighting app registrations tile on the Azure portal")

1. Select **New registration**.

    ![Screenshot of the app registration blade with the New registration button highlighted](media/sso-new-registration-teams.png "Screenshot of the app registration blade with the New registration button highlighted")
...
1. Leave the **Redirect URI** section blank for now, as you'll enter that information in the next steps. Select **Register**.
    ...
    ![Screenshot showing the registration form](media/sso-new-registration-details-teams.png "Screenshot showing the registration form")
...
1. After the registration is completed, it will open to the **Overview** page. Go to **Authentication** and then select **Add a platform**.
    ...
    ![Screenshot showing Add a platform under Authentication](media/sso-authentication-teams.png "Screenshot showing Add a platform under Authentication")

1. On the **configure platforms** blade, select **Web**. 

    ![Screenshot showing the web tile highlighted](media/sso-platform-web-teams.png "Screenshot showing the web tile highlighted")
...
1. Select **Configure** to confirm your changes.
    ...
    ![Screenshot showing the redirect URLs form](media/sso-add-redirect-url-teams.png "Screenshot showing the redirect URLs form")
...
1. Go to **API Permissions**. Select **Grant admin consent for \<your tenant name\>** and then **Yes**.
    ...
    ![Screenshot highlight the Grant admin consent for tenant-name button](media/sso-grant-consent-teams.png "Screenshot highlight the Grant admin consent for tenant-name button")
...
1. Open the app registration that you created [when you configured authentication](configuration-end-user-authentication-teams.md#use-azure-active-directory-as-your-identity-provider).
...
1. Go to **Expose an API** and select **Add a scope**.
    ...
    ![Screenshot showing Expose an api and then add a scope](media/sso-expose-an-api-scopes-teams.png "Screenshot showing Expose an api and then add a scope")
...
1. Enter a name for the scope, along with the display information that should be shown to users when they come to the single sign-on screen. Select **Add scope**.
    ...
    ![Screenshot showing the Add scope blade](media/sso-add-scope-bladed-teams.png "Screenshot showing the Add scope blade")
...
1. Select **Manage** on the side navigation pane, and then go to the **Authentication** tab.
    ...
    ![Go to Manage and then Authentication](media/auth-manage-sm-teams.png)
...
1. Enter the full scope URI from the **Expose an API** blade for the canvas app registration in the **Token exchange URL** field. This will be in the format of `api://1234-4567/scope.name`.
    ...
    ![Screenshot highlighting the scope's API](media/sso-api-teams.png "Screenshot highlighting the scope's API")  

    ![Screenshot showing the authentication tab with location for the API](media/sso-pva-token-url-teams.png "Screenshot showing the authentication tab with location for the API")
...
2. Update `clientId` with the **Application (client) ID** for the canvas app registration. Replace `<Directory ID>` with the **Directory (tenant) ID**. You get these IDs from the **Overview** page for the canvas app registration.
    ...
    ![App registration overview highlighting the Application and Directory IDs](media/sso-app-client-id-teams.png "App registration overview highlighting the Application and Directory IDs")
...
5. Update `<BOT ID>` with your bot's ID. You can see your bot's ID by going to the **Channels tab** for the bot you're using, and selecting **Mobile app** on the Power Virtual Agents portal.
    ...
    ![Bot ID shown on the Mobile app channel configuration page](media/sso-pva-botid-teams.png "Bot ID shown on the Mobile app channel configuration page")
