...
You call flows from within topics, as a discrete **Call an action** node. You can utilize flows that have already been created in your [Power Apps environment](environments-first-run-experience-teams.md), or you can create a flow from within the Power Virtual Agents [authoring canvas](authoring-create-edit-topics-teams.md).
...
![Power Automate flow template](media/PVAConnectorTemplate-teams.png)
...
![Power Virtual Agents trigger input types](media/PVAConnector_Inputs_1-teams.png)
...
![Power Virtual Agents trigger inputs](media/PVAConnector_Inputs_2-teams.png)
...
![Power Virtual Agents response output types](media/PVAConnector_Output_1-teams.png)
...
![Power Virtual Agents response outputs](media/PVAConnector_Output_2-teams.png)
...
![Power Automate flow template - Save](media/PVAConnectorTemplate_SAVE-teams.png)
...
![New flow shows up in Action picker](media/FlowInActionPicker-teams.png)
...
1. To create a new solution, select **New solution**.
    ...
    ![Create a solution](media/NewSolution-teams.png)
...
1. Give your new solution a name, select **CDS Default Publisher** in the **Publisher** field, enter a **Version** number, and then select **Create**. 
    ...
    ![Save a new solution](media/NewSolution_details-teams.png)


1. On the **Solutions** tab, go to the solution you want to use. Select **Add existing** to add a flow.

    ![Add existing menu](media/AddExistingFlow-teams.png)
...
1. On the **Add existing flow** page, select the **From solutions** tab, and then select your flow. Select **Add** to add your flow to the solution.
    ...
    ![Add flow to a solution](media/move-flow-from-solution-teams.png)
   ...
   To move a flow from **My flows** tab to a solution, select the **Outside solutions** option. Select **Add** to add your flow to the solution.
    ...
    ![Add flow from outside solutions](media/AddExistingFlow_details-teams.png)
...
1. In Power Virtual Agents, you can open a flow by using the flow's **View flow details** link on the list of available actions when you use the **Call an action node** in the authoring canvas.
    ...
    ![Modify a flow from Action Picker](media/ModifyFlowInPicker-teams.png)
    ...
    ![Modify a flow from Action node](media/ModifyFlowInAction-teams.png)
    ...
    ![Edit your flow using the Details page](media/FlowEditDetailsPage-teams.png)
    ...
    ![Open your flow for editing](media/EditFlow-teams.png)
...
1. A new **MSN Weather Connector** is added to the flow. Under **Location**, select **Add dynamic content**. Select **City** and **Zipcode** from the list. 
    ...
    ![Pass flow's input parameters to MSN Weather connector as location](media/AddLocationForMSN-teams.png)
...
1. In the response node **Return value(s) to Power Virtual Agents**, add the output parameters that you want to return to the bot. **Save** your flow. 
    ...
    ![Add dynamic variables to the flow's response](media/AddDynamicVariables-teams.png)
...
1. In Power Virtual Agents, go to the [**Topics page**](authoring-create-edit-topics-teams.md) for the bot you want to edit.
...
1. Go to the **Authoring canvas** for the new topic.
    ...
    ![Create a new Topic](media/CreateTopic-teams.png)
...
1. Add two new **Ask a question** nodes to ask users for the **City (String)** and **Zipcode (Number)** inputs.
    ...
    ![Add Topic Dialog questions](media/TopicDialogQuestions-teams.png)
...
1. Select the plus (**+**) button under the question nodes to add a new node. In the node selection window, select **Call an action**, and then select the flow you created earlier called **Get weather forecast**. 
    ...
    ![Call action](media/SelectFlowGetWeatherForecast-teams.png)
...
1. Under the flow's node, add a **Message** node and then enter a message that uses the flow’s outputs. For example:
    ...
    **Today's forecast for `(x)location`:`{x}day_summary`. 
    ...
    ![Call action](media/ActionNodeGetWeatherForecast-teams.png)
...
![Test Dialog](media/GetWeatherE2E-teams.png)
...
1. At the top of the **Test chat** pane, select the menu icon (three vertical periods) and then **Save snapshot**.
    ...
    ![Save snapshot](media/save-snapshot-teams.png)

2. In the window that appears, select **Save**.

    ![Save snapshot message](media/save-snapshot-window-teams.png)

3. The bot's content and conversational diagnostics will be saved in a .zip archive file called *DSAT.zip*.

    ![Conversational diagnostics archive](media/dsat-teams.png)
...
4. The *DSAT.zip* file contains two files: *botContent.json* and *dialog.json*. 
    ...
    1. *dialog.json* contains conversational diagnostics, including detailed descriptions of errors.
        ...
        ![Conversational diagnostics file](media/conversation-file-teams.png)
...
![Flag an issue](media/flag-issue-teams.png)
