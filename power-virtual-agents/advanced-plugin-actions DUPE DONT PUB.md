...
By default, plugins are only visible and usable in Power Virtual Agent or Copilot Studio by the person
Use AI plugins 

Share AI plugins 

By default, plugins are only visible and usable in Power Virtual Agent or Copilot Studio by the person who created them. Though, they can be shared so other users of the environment or groups can use it in their Copilot. The authorization is managed at the underlying Power Platform object level. 
...
Use AI plugins with Power Virtual Agent 
AI plugins can be used with Power Virtual Agent using the chatbot control in Power Apps.
This can be performed following these steps:
•	Navigate the Power Virtual Agent home page.
•	Create a new chatbot or select an existing one.
•	Select Topics in the left navigation.
•	Select + Add and Plugin action (preview)
 
•	Use the search box to find your plugin and select it in the search results. You can filter for plugin types such as connectors, flow, custom connectors etc. here.
•	Select Next
 
•	Select Finish
•	In Power Apps studio, insert the chatbot control from the Insert menu. See Add chatbot to a page.
Your Copilot is now ready to be used in your application. When end users will ask questions that one of the plugins you have added can address, Copilot will gather the necessary information for the plugin to work through multi-turn conversations and will display the results in the chatbot control.

Use AI plugins in Copilot studio 
AI plugins can be used with Copilot Studio using the Copilot control in Power Apps.
TODO
