...
> [!CAUTION]
> Content analysis and retrieval happens on behalf of the signed-in user, meaning this feature [requires user authentication to be configured for your bot using Azure AD under "Manual (for custom website) authentication option](nlu-boost-node.md#authentication). "No authentication" and "Only for Teams" authentication options will not be able to retrieve information from Sharepoint or OneDrive for Business.
>  
