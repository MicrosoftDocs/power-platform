...
description: Configure authentication with your identity provider to enable users to sign in when having a copilot conversation.
...
>[!NOTE]
> In Microsoft Teams, you can configure a Microsoft Copilot Studio copilot to provide authentication capabilities, so that users can sign in with a Microsoft Entra ID or any [OAuth2 identity provider](/azure/active-directory/develop/v2-oauth2-auth-code-flow), such as a Microsoft or Facebook account. 

You can [add user authentication to your copilot](advanced-end-user-authentication.md) when you edit a topic.
...
>
> Additionally, the **Only for Teams and Power Apps** option is not available if your copilot is integrated with [Dynamics 365 Customer Service](configuration-hand-off-omnichannel.md).
...
Only the Teams channel is available if you select this option. If you need other channels but still want authentication for your copilot (such as when [using generative AI features](nlu-gpt-overview.md), choose **Manual** authentication.
...
For more information about these variables and how to use them, see [Add user authentication to a Microsoft Copilot Studio copilot](advanced-end-user-authentication.md#authentication-variables).
...
Your copilot's authentication and **Require user to sign in** setting in combination determines whether you can [share the copilot](admin-share-bots.md) to control who in your organization can chat with it. The authentication setting doesn't affect sharing a copilot for collaboration.
...
- **Only for Teams**: The copilot works only on [the Teams channel](publication-add-bot-to-microsoft-teams.md). Since the user is always signed in, the **Require users to sign in** setting is turned on and can't be turned off. You can use copilot sharing to control who in your organization can chat with the copilot.
...
:::image type="content" source="media/configuration-end-user-authentication/auth-allow-everyone-chat-with-bot.PNG" alt-text="Screenshot of a message stating everyone in the organization can chat with the copilot because of its authentication setting.":::
