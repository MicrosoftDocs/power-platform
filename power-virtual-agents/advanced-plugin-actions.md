...
You can extend the capabilities of your bot by adding one or more plugin actions. Plugin Actions can be used by your copilot to respond to users automatically, using [Generative Actions](./advanced-generative-actions.md), or you can call them explicitly from within a Topic. 
...
Plugin Actions are also capable of generating a contextual response to a user's query, using the results of the action. Alternatively you can choose to explicitly author a response for the Plugin Action.
