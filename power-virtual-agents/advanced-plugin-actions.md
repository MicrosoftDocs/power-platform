...
## Authentication required for AI plugins

AI Plugins are always executed in the Microsoft Copilot runtime in the end-user context and can never be executed unauthenticated. Two types of authentication methods are supported supported.

1. **Integrated Auth (Teams and Power Apps)**: This authentication setup can be employed if your bot is being consumed from the Teams channel or if you are bot is embedded in any Power Platform surface which uses tokens built with the PPAPI standard.
