...
The following illustration shows how a user is signed in without seeing a login prompt (SSO) in Power Virtual Agents:
  ...
  :::image type="content" source="media/sso-illustration.png" alt-text="Illustration of SSO authentication flow.":::
...
1. Go to [App registrations](https://portal.azure.com/#blade/Microsoft_AAD_RegisteredApps/ApplicationsListBlade), either by selecting the icon or searching in the top search bar.
    ...
    :::image type="content" source="media/sso-app-registrations.png "Screenshot highlighting app registrations tile on the Azure portal"" alt-text="Screenshot highlighting app registrations tile on the Azure portal." border="false":::

1. Select **New registration**.

    :::image type="content" source="media/sso-new-registration.png "Screenshot of the app registration blade with the New registration button highlighted"" alt-text="Screenshot of the app registration blade with the New registration button highlighted." border="false":::
...
1. Leave the **Redirect URI** section blank for now, as you'll enter that information in the next steps. Select **Register**.
    ...
    :::image type="content" source="media/sso-new-registration-details.png "Screenshot showing the registration form"" alt-text="Screenshot showing the registration form." border="false":::
...
1. With the registration open, go to **Authentication** and then select **Add a platform**.
    ...
    :::image type="content" source="media/sso-authentication.png "Screenshot showing Add a platform under Authentication"" alt-text="Screenshot showing Add a platform under Authentication." border="false":::

1. On the **configure platforms** blade, select **Web**. 

    :::image type="content" source="media/sso-platform-web.png "Screenshot showing the web tile highlighted"" alt-text="Screenshot showing the web tile highlighted." border="false":::
...
1. Select **Configure** to confirm your changes.
    ...
    :::image type="content" source="media/sso-add-redirect-url.png "Screenshot showing the redirect URLs form"" alt-text="Screenshot showing the redirect URLs form." border="false":::
...
1. Go to **API Permissions**. Select **Grant admin consent for \<your tenant name\>** and then **Yes**.
    ...
    :::image type="content" source="media/sso-grant-consent.png "Screenshot highlight the Grant admin consent for tenant-name button"" alt-text="Screenshot highlight the Grant admin consent for tenant-name button." border="false":::
...
1. Go to **Expose an API** and select **Add a scope**.
    ...
    :::image type="content" source="media/sso-expose-an-api-scopes.png "Screenshot showing Expose an api and then add a scope"" alt-text="Screenshot showing Expose an api and then add a scope." border="false":::
...
1. Enter a name for the scope, along with the display information that should be shown to users when they come to the single sign-on screen. Select **Add scope**.
    ...
    :::image type="content" source="media/sso-add-scope-bladed.png "Screenshot showing the Add scope blade"" alt-text="Screenshot showing the Add scope blade." border="false":::
...
1. Select **Manage** on the side navigation pane, and then go to the **Authentication** tab.
    ...
    :::image type="content" source="media/auth-manage-sm.png" alt-text="Go to Manage and then Authentication." border="false":::
...
1. Enter the full scope URI from the **Expose an API** blade for the bot’s authentication app registration in the **Token exchange URL** field. The URI will be in the format of `api://1234-4567/scope.name`.
    ...
    :::image type="content" source="media/sso-api.png "Screenshot highlighting the scope's API"" alt-text="Screenshot highlighting the scope's API." border="false":::  

    :::image type="content" source="media/sso-pva-token-url.png "Screenshot showing the authentication tab with location for the API"" alt-text="Screenshot showing the authentication tab with location for the API." border="false":::
...
2. Update `clientId` with the **Application (client) ID** for the canvas app registration. Replace `<Directory ID>` with the **Directory (tenant) ID**. You get these IDs from the **Overview** page for the canvas app registration.
    ...
    :::image type="content" source="media/sso-app-client-id.png "App registration overview highlighting the Application and Directory IDs"" alt-text="App registration overview highlighting the Application and Directory IDs." border="false":::
...
5. Update `<BOT ID>` with your bot's ID. You can see your bot's ID by going to the **Channels tab** for the bot you're using, and selecting **Mobile app** on the Power Virtual Agents portal.
    ...
    :::image type="content" source="media/sso-pva-botid.png "Bot ID shown on the Mobile app channel configuration page"" alt-text="Bot ID shown on the Mobile app channel configuration page." border="false":::
