...
Follow the steps in [Configure user authentication](configuration-end-user-authentication#use-azure-active-directory-as-your-identity-provider) topic to create Azure AD application.
...
> [!IMPORTANT]
> To avoid users from having to consent to each application, a Global Administrator, Application Administrator, or a Cloud Application Administrator must [grant tenant-wide](/azure/active-directory/manage-apps/grant-admin-consent) consent to your app registrations.
...
You'll now need to update the Azure AD authentication settings in the Power Virtual Agents web app that you configured as part of the [prerequisites](configure-sso-teams#prerequisites).
...
1. Go to the Azure AD authentication you configured when following the steps to [create the new Azure AD authentication in the Power Virtual Agents web app](configuration-end-user-authentication#configure-manual-authentication).

1. Add the Token Exchange URL as described in the [Configure SSO with Azure AD](configure-sso#configure-authentication-in-power-virtual-agents-to-enable-sso) topic, under the **Configure authentication in Power Virtual Agents to Enable SSO** heading.
...
These steps update the manifest file for the bot. Now you can download the manifest file (as a .zip file) and upload to Teams for test or distribution, or submit for your admin approval. For more information, see the [Add bot to Microsoft Teams in Teams](publication-add-bot-to-microsoft-teams) topic.

1. Select **Availability options**.

1. Select **Download .zip** to get the new manifest.

:::image type="content" source="media/configure-sso-teams/image10.png" alt-text="Screenshot displaying Download .zip button highlighted.":::
