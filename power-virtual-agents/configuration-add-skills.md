...
You can use pro-code tools to build Azure Bot Framework Skills and embed those into any topic within a Power Virtual Agents bot.
...
## Sample skills
You can start using some [sample Skills provided by the Azure Bot Framework](/azure/bot-service/bot-builder-skills-overview?view=azure-bot-service-4.0#bot-framework-skills).
...
You'll then need to register the Skill.

First, you'll need provide your bot's ID to your Skill developer prior to being authorized to add a new Skill.

**Add your bot to Skill's allow list:**

1. In the [Power Virtual Agents portal](https://powerva.microsoft.com), cn the side navigation pane, expand the **Manage** menu and select **Skills**.
...
Skill manifest endpoint mismatch|MANIFEST_ENDPOINT_ORIGIN_MISMATCH(206)|Attacker altered manifest and hosted on his website (see also)
...
Skill language check|N/A (english only, skills and designer)|Skill Description is available in the language of the bot author
...
Acquire token to call skill|AADERROR_OTHER(601)|Other AAD issue - CCI AAD tenant is down
Health check successful|ENDPOINT_HEALTHCHECK_FAILED(700)|Network errors, http errors (not found, not authenticated); Unauthorized with mean skill has not whitelisted bot appId (most of the time, but the skill developer could auth the bot in other ways - eg: by tenantId)
Too many messages|TRUNCATED_TOO_MANY_ENTRIES (800)|Should be unreachable, since only ~20 messages are defined, and ~10 could occur in parallel, limit is 100
