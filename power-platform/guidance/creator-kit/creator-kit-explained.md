...
> [!IMPORTANT]
> A Theme JSON object is a set of colors that are mapped to properties in the components. The code block  generated by the Theme Generator app and should be stored in a global variable to be referenced by the components that accept it. Any component, including those outside of the Creator Kit, can reference these values.
...
1. Save the Theme JSON as a global variable in the OnStart property of your app, with the palette as a property of the object variable (shown below):
    ```powerapps-dot
    Set( AppTheme, {
      palette: {
        themePrimary: "#0078d4",
...
Any control, including those outside of the Creator Kit, can reference the theme colors, but the specific property of palette needs to be indicated.

Example formula for a standard button's `TextColor`:
```powerapps-dot
