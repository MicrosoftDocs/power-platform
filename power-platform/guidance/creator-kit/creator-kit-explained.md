...
> [!IMPORTANT]
> A Theme JSON object is a set of colors that are mapped to properties in the components. The code block  generated by the Theme Generator app and should be stored in a global variable to be referenced by the components that accept it. Any component, including those outside of the Creator Kit, can reference these values to maintain consistency across all app components.
