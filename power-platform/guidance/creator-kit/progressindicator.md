...
Progress indicators are used to show the completion status of an operation lasting more than two seconds. If the state of progress cannot be determined, use a Spinner instead. Progress indicators can appear in a new panel, a flyout, under the UI initiating the operation, or even replacing the initiating UI, as long as the UI can return if the operation is canceled or is stopped.
...
## Best Practices
Refer to [Fluent UI ProgressIndicator control best practices](https://developer.microsoft.com/fluentui#/controls/web/ProgressIndicator)
