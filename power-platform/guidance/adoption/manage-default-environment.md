...
Like every other environment type (sandbox, production, developer), the default environment also gets backed up. However, there's no manual backup that you can request on your own, for the default environment. Like any other environment, a restore operation is possible and requires you to connect with Microsoft Support. Be aware that when you restore the default environment, you need to consider you might also be restoring all the unused or abandoned apps and flows that were removed after the cleanup.
