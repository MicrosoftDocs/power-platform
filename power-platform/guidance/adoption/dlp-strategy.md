---
title: Implement a data policy strategy
description: Learn how to implement a data policy strategy in Power Platform to prevent unintentional data exposure or data loss and protect organizational security.
#customer intent: As a Power Platform admin, I want to establish data policies so that I can prevent users from unintentionally exposing organizational data.
...
Power Platform allows makers to create and share [custom connectors](/connectors/custom-connectors/). You can manage [custom connectors for tenant and environment level data policies](/power-platform/admin/dlp-custom-connector-parity). 
...
Data policies let you govern how agents connect and interact with data and services, within and outside your organization. Learn more in [Configure data policies for agents](/microsoft-copilot-studio/admin-data-loss-prevention).

### Data policies for desktop flows

Power Automate allows you to create and enforce data policies that classify desktop flow modules and individual module actions as business, non-business, or blocked. This categorization prevents makers from combining modules and actions from different categories into a desktop flow or between a cloud flow and the desktop flows it uses. Learn more in [Data policies for desktop flows](/power-automate/prevent-data-loss#data-loss-prevention-for-desktop-flows).
...
We recommend the following starting point for data policies for **shared user and team productivity environments**:
...
- Create a minimal number of policies per environment. There's no strict hierarchy between tenant and environment policies. At design and runtime, all policies that are applicable to the environment in which the app or flow resides are evaluated together to decide whether the resource is in compliance or in violation of data policies. [Multiple data policies](/power-platform/admin/dlp-combined-effect-multiple-policies) applied to one environment will fragment your connector space in complicated ways and might make it difficult to understand issues your makers are facing.  
...
Let's look at how Contoso Corporation, our sample organization for this guidance, set up their data policies. The setup of their data policies ties in closely with their [environment strategy](environment-strategy.md).
...
:::image type="content" source="media/dlp-strategy_.png" alt-text="Diagram illustrating Contoso's environment and data policy strategy." lightbox="media/dlp-strategy_.png" :::
...
1. Use the [DLP SDK](../../admin/data-loss-prevention-sdk.md) to add custom connectors to a data policy.
