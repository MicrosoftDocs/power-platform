...
### User settings
  ...
  | canvasshare.aadGroupId.canvas-app-schema-name | No | Sets the Microsoft Azure Active Directory (Azure AD) group ID for sharing the canvas app. When the canvas app is to be shared with multiple Azure AD groups, a unique identifier of .1 or .2 can be added to the end of the setting. |
  | canvasshare.roleName.canvas-app-schema-name | No | Sets the role to assign to the Azure AD group. Valid options are **CanView**, **CanViewAndShare**, and **CanEdit**. When the canvas app is to be shared with multiple Azure AD groups, a unique identifier of .1 or .2 can be added to the end of the setting. |
  ...
  | flow.sharing.flow-name.flow-id  | No | Sets the Dataverse Azure AD Group Team to share the process record associated with the flow after the solution is imported into the deployment environment. |
  ...
  | connector.teamname.connector-name.connector-id | No | Sets the Dataverse Azure AD Group Team to share the connector record associated with the custom connector after the solution is imported into the deployment environment. |
