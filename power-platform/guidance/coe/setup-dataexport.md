...
If your inventory comes from [Data Export](setup.md#what-data-source-should-i-use-for-my-power-platform-inventory), you can configure dataflows as part of the setup.
...
> - When you see data in your storage account set up to receive data from Data Export, use these Data Export configuration steps. This configuration might show up to five days from the initial setup of Data Export in the Power Platform admin center.
...
1. Update the `DatalakeURL` parameter with the link to your Data Lake Storage URL and the `EnvironmentAPI` parameter with the link to your Environment Web API endpoint.
   ...
   The **CoE BYODL Makers** start refreshing. Wait for the refresh to finish.
...
This section provides guidance on how to resolve common issues that might arise during the setup and configuration of the Data Export.

### The specified path doesn't exist

This dataSource error means not all required folders and files are available yet in the storage account, since you recently configured the Data Export. After the initial setup, updated data can take up to five days for folders to populate.
...
This section includes the full list of environment variables that affect the Data Export methodology to inventory the tenant. The Setup Wizard fills in these values for you.
