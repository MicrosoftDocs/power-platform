...
### User Settings (array of pipeline variables)
  ...
  | activateflow.order.flow-name.flow-id                                    | No       | The order in which to activate the flow. This is useful when activating parent / child flows. However, the pipelines will attempt to activate flows as long as there are flows to activate if this isn't set.  |
  | activateflow.activate.flow-name.flow-id                                 | No       | Whether or not activate this flow. In certain cases flows may intentionally be left in an off state in a deployment environment. This will trigger the pipelines to either turn on the flow or not when deployed. |
  | connector.teamname.connector-name.connector-id                          | No       | The Dataverse AAD Group Team to share the connector record associated with the custom connector after import of the solution in the deployment environment. |
