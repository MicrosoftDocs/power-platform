...
| CLEANUP - Admin \| Sync Template v3 (Delete Bad Data) | Scheduled | Daily | This flow runs daily, and looks for data in the inventory that is not complete, for example flows without an environment. These dead rows are removed from the inventory. |
...
| Command Center > Get M365 Service Messages | Instant | from Admin - Command Center canvas apps | this flow connects to [Microsoft Graph](/graph/api/serviceannouncement-list-messages?view=graph-rest-1.0&tabs=http) to list Power Platform Message Center service updates and returns them to the Admin - Command Center canvas app. |
