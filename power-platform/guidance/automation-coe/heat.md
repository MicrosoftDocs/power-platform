...
Power Automate provides environments of different types (production, sandbox, trial).  Each environment has a defined set of users and roles. Admins can bring their own compute infrastructure to install Power Automate for desktop and the necessary software.
...
Watch the video series: [SAP GUI based RPA in Power Automate Desktop](https://youtu.be/7u9TYcDShNA)
