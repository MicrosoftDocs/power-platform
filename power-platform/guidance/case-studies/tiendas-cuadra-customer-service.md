...
Tiendas CUADRA partnered with Microsoft partner Algoritmo to build a multi-agent solution, using Microsoft Copilot Studio, integrated with Dynamics 365 Customer Service to support escalation to customer service representatives. The solution also integrates with the company’s e‑commerce platform to enable order lookup and product discovery scenarios.
...
The solution has made customer service smoother and more reliable. It’s available at all times, responds quickly even when many people are reaching out, and handles routine questions so customer service representatives can focus on the issues that truly need their attention.
...
Team confidence in using AI has also increased, opening the door to additional use cases across the organization.
...
- Acts as the first point of contact for customer
...
Customers visit Tiendas CUADRA’s website and start conversations with the agent. The agent has preconfigured options and also allows submitting questions using natural language, such as *“How should I care for this type of leather?”* or *“Do you have any current promotions on leather boots?”*
...
If the order has been shipped, the response includes a shipment tracking link that directs the customer to the carrier’s tracking page. If the order cannot be found, for example due to an invalid order number or mismatched customer details, the customer is offered the option to submit a request for assistance.
...
For product discovery, customers can upload an image of a product they are interested in. The agent guides the customer through the process and uses the uploaded image as input to identify complementary products, such as matching apparel and accessories.
...
The agent uses image‑based product recognition to generate outfit suggestions and recommend complementary products. Generated images are provided for illustrative purposes and may not represent exact product combinations or final purchasable sets.
...
AI prompts are also used to generate concise summaries for order status and product‑related inquiries. Rather than returning raw system data to the customer, relevant information, such as customer intent, order details, or product context, is processed through AI prompts to produce clear and easy‑to‑understand responses before they are shown to the customer.
...
One key challenge during implementation was credit consumption. Early versions relied heavily on generative AI for most responses, which increased overall cost. The team addressed this by refining prompts, selectively applying generative AI, and optimizing response length. As a result, responses became more concise and cost‑efficient without compromising quality.
