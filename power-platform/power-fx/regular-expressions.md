...
| Unicode code point | `\u{01F47B}` with up to 8 hexadecimal digits. Must be in the range 0 to U+10FFFF and can't be used for either a high or low surrogate. May result in a surrogate pair (two characters) if greater than U+FFFF. |
...
`\v` isn't supported as it ambiguous across regular expression languages. Use `\x0b` for a vertical tab or `[\x0b\f\r\n\x85\u2028\u2029]` for vertical whitespace.
...
| Greedy between n and m | `{n,m}` matches between *n* and *m* times, with as *large* a match as possible. For example, `a{1,3}` matches the first 3 characters of `aaaaa`. |
...
| Exact n | `{n}` matches *n* times, exactly. For example, `a{3}` matches exactly 3 characters of `aaaaa`. |

Possessive quantifiers aren't supported.

Mixing sub-matches and quantifiers has limitations. See [Possibly empty sub-matches](#possibly-empty-submatches) for more information.
...
Inline options can't be used to disable an option or set an option for a sub-expression.
...
In this example, only the "Hello" is matched as by default the `.` will not match the newline:
...
- Characters classes aren't included in these changes. Space characters and `#` act as they normally do. For example, `IsMatch( "a#b c", "(?x)a[ #]b[ #]c" )` returns *true*. Some regular expression languages include character classes in free spacing, or provide an option to include them, but Power Fx does not.
...
Named and numbered submatches can't be used together. Some implementations treat a mix of numbered and named capture groups differently which is why Power Fx disallows it. 

## Possibly empty submatches

As stated in the introduction, Power Fx's regular expressions are intentionally limited to features that can be consistently implemented on top of .NET, JavaScript, and other programming language regular expression engines. Authoring time errors prevent the use of features that aren't a part of this set. 
...
To avoid different results across Power Fx implementations, submatches that could be empty can't be used with a quantifier. Here are examples of how a submatch could be empty:
...
The submatch in `(?<submatch>a+)+` can't be empty, as there must be at least one `a` in he submatch, and is supported.
