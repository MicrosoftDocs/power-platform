...
- Count unique lookups per table (ignore out-of-the-box fields like `Created By` and other table looks up if not migrating).
...
This approach defines the sequence of data migration load and works well in most scenarios. For more complex cases, consider:
...
1. Choose the right tool: Select a tool based on data size and complexity:
...
1. **Update the main table with GUIDs**:
    ...
    - Update the Processing Flag to mark records as:
        ...
        - E – Errored
        - U – Unprocessed
    - This enables efficient reruns by skipping already processed records and supports lookup resolution in subsequent loads.
...
- Use a separate Dataverse environment. Less common, but useful if you want to isolate archived data. This environment can be retired later to simplify cutover planning.
