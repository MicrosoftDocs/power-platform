...
3. Deploy the managed solution.
    ...
    1. Delete the unmanaged solution(s) in your test environment. To do this, go to [Power Apps](https://make.powerapps.com/?utm_source=padocs&utm_medium=linkinadoc&utm_campaign=referralsfromdoc), select the test or production environment, select **Solutions**, select the unmanaged solution, and then on the command bar, select **Delete**. Notice that deleting the managed unmanaged solution doesn't delete the solution's components.
    1. Import the solution into your test environment using [Power Platform CLI](/powerapps/developer/data-platform/powerapps-cli#solution) solution import commanding or the [DevOps tooling](devops-build-tool-tasks#solution-tasks.md) solution import task. Currently, you can't successfully import the converted solution using the Power Apps portal.
