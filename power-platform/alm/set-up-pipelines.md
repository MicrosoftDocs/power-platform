...
- In the deployment pipeline configuration app, **Share** the pipeline record with makers (or Azure Active Directory groups). **Read** permission is sufficient to view and run a pipeline from within their development environment. More information: [Share rows with a user or team](/power-apps/user/share-row)
