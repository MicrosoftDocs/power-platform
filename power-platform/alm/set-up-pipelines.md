...
Yes. As an admin, you can assign the **Deployment Pipeline Default** role to anyone you would like to grant lightweight pipeline creation permissions to. This role is **not** assigned to anyone by default in the case of custom host, so the lightweight personal pipeline creation experience is only visible by default in environments that are **not** already associated with a custom host.
