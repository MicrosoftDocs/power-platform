---
title: Set up OAuth 2.0 authentication for the SAP OData connector (preview)
...
This article walks you through how to set up the **OAuth 2.0** authentication method in the SAP OData connector for Power Platform. By using this method, the connector authenticates by using the [authorization code flow](https://oauth.net/2/grant-types/authorization-code/) against a generic OAuth 2.0 identity provider. Users sign in interactively, and the connector uses the resulting access token as a bearer token on OData calls to SAP.
...
- Admin access to the identity provider so you can register an OAuth 2.0 client application. For SAP IAS, this access is the [SAP Cloud Identity Services administration console](https://help.sap.com/docs/identity-authentication/identity-authentication/get-started).
...
This section lists *named values* to make the examples easier to follow. You reuse these values throughout the article and reference them in later sections. When you follow the examples in this article, replace the named values with your own values.
...
In short, you register the client, add the redirect URI to the allow list, and enter the identity provider's endpoints and credentials in the connection.
...
## Step 3: Get the connector redirect URI and add it to the allow list
...
2. In the SAP IAS application, go to **Single Sign-On** > **OpenID Connect Configuration** (or the application's **Redirect URIs** setting), add the `Redirect URI`, and save it.
...
2. Create a new manual-trigger **flow**.
...
> [!TIP]
>
> - If the connection reports that it isn't authenticated, confirm that you added the redirect URI to the allow list exactly, entered the client secret in full, and configured the OAuth client as a confidential client with the authorization code and refresh token grant types enabled.
