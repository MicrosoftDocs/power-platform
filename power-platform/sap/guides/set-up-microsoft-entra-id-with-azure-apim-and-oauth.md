---
title: Set up Microsoft Entra ID, Azure API Management and SAP for single sign-on from Microsoft Power Platform SAP OData connector
description:  This guide walks you through setting up SAP and Azure API management so your users can access SAP data and make OData calls in Microsoft Power Platform using their Microsoft Entra ID for authentication.​
author: scottwoodallmsft
ms.author: scottwoodall
ms.service: power-platform
ms.subservice: sapodata-connector
ms.topic: how-to
ms.date: 08/23/2024
---
...
1. Choose `Enable SAML 2.0 Support` if this step has never been taken before.
1. Enter a URI compliant `Provider Name`. Microsoft Entra ID requires this to be URI compliant. If `Provider Name` is already configured and is not URI compliant, consult your SAP Basis team for next steps. This is potentially a breaking change if `Provider Name` has to be changed so coordinate internally before changing. Steps for changing this value is out of scope for this document.
1. Select the `Metadata` button, then select the `Download Metadata` button to download the SAP SAML metadata. This will be uploaded to Microsoft Entra ID.

## Create a Microsoft Entra ID Enterprise application

We create a Microsoft Entra ID Enterprise application that will be the SAP system.
...
1. Change the `Sign on URL` to a URI compliant value. This parameter is not used and can be set to anything.
1. Select `Save`.
...
With SAP and Microsoft Entra ID pieces being setup, we can configure SAP to trust our Microsoft Entra ID Enterprise application.
...
1. Select `Finish`.
1. Select the Microsoft Entra ID entry amongst the `List of Trusted Providers` that was just added.
...
We set up a Microsoft Entra ID application that will grant access to the Microsoft Power Platform SAP OData app registration.  This will allow our Azure API Management resource to convert OAuth tokens to SAML ones.
...
1. Select `Authentication`, select `Add a platform` then select `Web`.
...
1. When a Microsoft Entra ID Enterprise application is created, it always creates a matching `App registration`. Find this matching `App registration` that is paired with the Entra ID Enterprise application that was created previously.
...
1. For the `Scope Assignment` step:
    1. Select `Add` and choose the OData services that Azure API Management will use and select `Next`
1. Select `Finish`.
...
1. Use [OData Transformer](https://convert.odata-openapi.net/) to convert your OData metadata XML file to an OpenAPI JSON specification and save this to your desktop.
...
| SAPOAuthScope                        | OData scope(s) chosen during SAP OAuth configuration |
...
Azure API Management has the capability, through policies, to convert our Microsoft Entra ID tokens into SAML tokens that SAP will accept.
