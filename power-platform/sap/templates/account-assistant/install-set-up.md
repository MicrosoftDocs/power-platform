...
To install and set up **Account assistant for SAP solutions**, review the prerequisites before you get started. Once you have all the requirements, follow the steps outlined in this article to install and set up the agent.
...
- **Allocate environment capacity**—**Account assistant for SAP solutions** requires Copilot Studio message capacity in the production environment where the agent is deployed.
    ...
    1. Expand **Resources** in the left pane and select **Capacity**.
    1. Select **Add-ons**.
...
1. Select **Import** to start the solution import process for the **Account assistant for SAP solutions**. When the import finishes, you receive a notification.
...
1. Select **Availability options**. You can share Account assistant for SAP solutions in several ways, including _share via link_, _deploy to the Teams app store_, or _distribute as a custom app file_. Choose your deployment method.
1. If you choose to deploy to the Teams app store, you must also decide how broadly to share **Account assistant for SAP solutions**.
    - Select **Show to everyone in my org** to deploy _Account assistant for SAP solutions_ to everyone in your organization. This option submits the agent in the Microsoft Teams admin center for approval by Global Administrators and Teams Administrators. After approval, it appears in the _Built for my org_ area of the Teams app catalog. To learn more, see [Manage your apps in the Microsoft Teams admin center](/microsoftteams/manage-apps).
...
> [!NOTE]
> Account assistant for SAP solutions is designed for a Microsoft Teams experience, and the default authentication is configured to **Authenticate with Microsoft**. To learn more, see [Add user authentication to topics](/microsoft-copilot-studio/advanced-end-user-authentication).
