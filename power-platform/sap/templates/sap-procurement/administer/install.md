...
You can download either managed or unmanaged solution files.
...
After you download **either** the *managed* or *unmanaged* solution files, install them into the developer (sandbox) environment that you prepared for them.
...
1. Browse for and select either the managed *or* unmanaged version of the **mpa_SAPBase** file you downloaded in the previous step.
...
1. Select **Import** to start the import process of the *SAP Base* solution. When the import is complete, you see a *Success* message.
...
1. Go to **Solutions** on the left pane of the *Power Apps maker portal* after the SAP Base solution installs successfully.
...
1. Select **Import** to start the import process of the *SAP Procurement* solution. When the import is complete, you see a *Success* message.

For more information, see [Solution concepts](/power-platform/alm/solution-concepts-alm).

> [!IMPORTANT]
>
> The template files contain two solutions. You need to install both solutions of the same type (two unmanaged or two managed) to your prepared environment:
>
> - **SAP Base solution** contains the base Power Platform components required for the SAP Procurement solutions. The base solution also supports other SAP-related solutions that Microsoft releases later.
