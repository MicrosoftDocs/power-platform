...
To obtain a bearer token with the correct permissions, complete the following steps:
...
1. Fill in the **Register an application** page:
    1. **Name** — Give the application a recognizable name, such as **Power Platform Admin SDK**.
...
1. Select **Register** to create the application. After registration completes, note the **Application (client) ID** and **Directory (tenant) ID** from the overview page — you need both values later.

You can also create the registration by using Azure CLI:
...
The command returns a JSON object. Note the `appId` value — this value is your client ID.

## Step 2. Configure API permissions
In your new app registration, go to the **Manage - API Permissions** tab. Under the **Configure permissions** section, select **Add a Permission**. In the dialog box, select the **APIs my organization uses** tab, and then search for **Power Platform API**. You might see several entries with a name similar to this one, so make sure you use the one with the GUID **8578e004-a5c6-46e7-913e-12f58912df43**.

If you don't see Power Platform API displayed in the list when searching by GUID, you might still have access to it but the visibility isn't refreshed. To force a refresh, run the following script:
...
From here, select the permissions you need. These permissions are grouped by [**Namespaces**](/rest/api/power-platform). Within a namespace, you see resource types and actions, such as *AppManagement.ApplicationPackages.Read*, which gives read permissions for application packages. For more information, see the [Permission reference](programmability-permission-reference.md) article.

> [!NOTE]
> Power Platform API uses delegated permissions only at this time. For applications that run with a user context, request delegated permissions by using the **scope** parameter. These permissions delegate the privileges of the signed-in user to your application, so it can act as the user when calling Power Platform API endpoints.
>
> For service principal identities, don't use application permissions. Instead, after you create your app registration, assign it an RBAC role to grant scoped permissions (such as **Contributor** or **Reader**). For more information, see [Tutorial: Assign RBAC roles to service principals](programmability-tutorial-rbac-role-assignment.md).

After you add the required permissions to the application, select **Grant admin consent** to complete the setup. By granting admin consent, you authorize the permissions for all users in the tenant so they aren't prompted with an interactive consent dialog the first time they use your app. If you prefer interactive consent per user, follow [Microsoft identity platform and OAuth 2.0 authorization code flow](/azure/active-directory/develop/v2-oauth2-auth-code-flow).

You can also grant admin consent by using Azure CLI:
...
1. Within your app registration, go to **Manage - Authentication**.
...
You can also add the redirect URI by using Azure CLI:
...
Under the **Advanced settings** section on the same **Authentication** tab, there's an **Allow public client flows** toggle. Set this toggle to **Yes** only if you plan to use the Resource Owner Password Credentials (ROPC) flow, which sends a username and password directly in the token request body. 
...
If your app requires reading and writing resources as itself, also known as a _service principal_, there are two ways to authenticate. To use certificates, go to **Manage - Certificates and secrets**. Under the **Certificates** section, upload an x509 certificate that you can use to authenticate. 

The other way is to use the **Secrets** section to generate a client secret. Save the secret in a safe location for use with your automation needs. The certificate or secret options allow you to authenticate with Microsoft Entra and receive a token for this client, which you pass along to either the REST APIs or PowerShell cmdlets.

## Step 5. Request an access token
You can obtain an access bearer token in two ways: one way is for username and password, and the other way is for service principals.
...
Be sure to read the [Configure certificates and secrets](#step-4-optional-configure-certificates-and-secrets) section. Then, send a POST request via HTTP to Microsoft Entra ID with a client secret payload. This authentication method is often referred to as service principal authentication.

> [!IMPORTANT]
> Before using service principal authentication, complete Steps 1-4 earlier in this article to create and configure your app registration with a certificate or client secret. Then assign the service principal an RBAC role to control its level of access. To learn more, see [Tutorial: Assign RBAC roles to service principals](programmability-tutorial-rbac-role-assignment.md).
...
After running these commands, you can use your app registration with the SDKs, PowerShell, or direct REST calls. To look up permission IDs for the `--api-permissions` parameter, see the [Permission reference](programmability-permission-reference.md).

## Troubleshooting common issues

### "Consent required" or "needs admin approval" errors
This error occurs when the admin hasn't consented to the API permissions on your app registration. Go to **App registrations** > your app > **API permissions** and select **Grant admin consent**. 
...
This error means the enterprise application associated with your app registration has **User assignment required** set to **Yes**. When this setting is enabled, only users or groups explicitly assigned to the application can sign in. To fix this error, take one of the following actions:

- Go to **Microsoft Entra ID** > **Enterprise applications** > your app > **Properties** and set **Assignment required** to **No**.
- Add the relevant users or security groups under **Users and groups**.

### Conditional access policies blocking access
If your organization applies conditional access policies, they might block token acquisition for your app registration. Common causes include device compliance requirements, location restrictions, or risk-based policies. Work with your Microsoft Entra administrator to either exclude your app registration from the policy or ensure clients meet the policy requirements.

### "Power Platform API" not found in the API picker
If searching for **Power Platform API** by name or GUID in the API permissions dialog returns no results, the service principal isn't created in your tenant. Follow the force-refresh steps in [Step 2](#step-2-configure-api-permissions) to create it.

## Authenticate with Power Platform SDKs and PowerShell

The following examples show how to authenticate and make a sample API call by using each SDK and PowerShell. Before running these examples, complete Steps 1-3 earlier in this article to create and configure your app registration.

### Interactive authentication (delegated user)

Interactive authentication opens a browser window for the user to sign in. This flow works best for developer scripts, admin tools, and any scenario where a user is present.
...
Confidential client authentication uses a client secret or certificate and doesn't require user interaction. This authentication flow is best for background services, pipelines, and automation.
