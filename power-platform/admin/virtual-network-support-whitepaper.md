...
> [!NOTE]
> If only configuring network security groups, without configuring the next hop (that is, attaching a NAT Gateway or adding a custom routing table) traffic is restricted according to the rules specified. However, internet-bound traffic will still agress from Power Platform owned IP addresses. 
