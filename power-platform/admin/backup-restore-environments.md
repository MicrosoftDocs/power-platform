...
The restore process, especially for environments with large amounts of data, is a complex backend operation. If the restore operation fails, the target environment will be left in a disabled state. To retry restore, the failed environment must be the target environment for the operation. Please wait 30 minutes and retry the operation again. The other actions you can take for the disabled target environment are reset, delete or copy to as a target environment.
