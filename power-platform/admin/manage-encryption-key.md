...
> - After the feature is turned on in your tenant, all new environments are created with Azure SQL storage only. These environments, regardless of whether they are encrypted with your BYOK or Microsoft managed key, have restrictions with File upload size, cannot use Cosmos and Datalake services, and Dataverse Search indexes are encrypted with Microsoft managed key. To use these services, you will need to [migrate to Customer managed key](https://learn.microsoft.com/power-platform/admin/cmk-migrate-from-byok).
