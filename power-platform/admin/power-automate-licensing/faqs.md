...
If a flow is set to the Power Automate Process plan, then it can access premium connectors and has the highest limits and always uses the process plan request limits, regardless of who runs the flow.
...
### How many Process licenses do i need, if i multiple cloud flows, each of them invoking its own RPA flows in unattended mode on a machine?

With one Process license, you can run multiple cloud flows by queueing them. For example, if have four (3) independent cloud flows, and each one of them invokes its own desktop flow running on the same machine, you can run the first one at 09:00 AM, the second one at 11:00 AM, and so on, in the same virtual machine. 
In this image, all cloud flows are covered by one Process license as they have a desktop flow running on the same machine. Any parent, child flows, in context flows of these cloud flows are also covered by the Process license on the machine.

![image](https://github.com/MicrosoftDocs/power-platform-pr/assets/62711514/9f30ee5e-da66-4c9d-910b-d3409215ffcc)

If you need flows to run in parallel, you need multiple Process licenses. For example, out of the 3 cloud flows, you need 2 of them to run in parallel, you need 2 Process licenses. 
...
Once a license is assigned/flow is associated to an app, it may take up to 7 days for the premium license to appear in the Power Automate portal. To refresh the latest license status, edit and save a flow. 
