...
2. Access to [standard connectors](#standardconnectors)
3. 2K Power [platform requests/day](#APIrequests) (10K requests/day during [transition period](#Transition_period))

The following Power Automate capabilities are not included:

1. Access to [premium connectors](#premiumconnectors) is not included except in Dataverse for Teams environments. Learn more about Dataverse for Teams capabilities [here](https://docs.microsoft.com/en-us/power-platform/admin/pricing-billing-skus#dataverse-capabilities-with-microsoft-365-licenses)
2. [Business Process Flows](#businessprocessflow)
3. [Custom connectors](#customconnectors)
4. [On premises gateways](#onpremisegateway)
5. [Robotic process automation](#RPA)
6. [AI builder capacity](#AIbuildercredits)
...
2.What Power Automate capabilities are included in Dynamics licenses

Dynamics licenses will include following Power Automate capabilities:

1. Create and execute Automated/Scheduled/Button flows
2. Access to [standard connectors](#standardconnectors)
3. Access to [premium connectors](#premiumconnectors) within app context
4. [Business Process Flows](#businessprocessflow)[within app context](#inappcontext)
5. [Custom connectors](#customconnectors)[within app context](#inappcontext)
6. [On premises gateways](#onpremisegateway)[within app context](#inappcontext)
7. [Power platform request](#APIrequests) limits: Dynamics Team member gets 5K requests/day (25K requests/day during [transition period](#Transition_period)), Dynamics professional gets 10K requests/day (100K requests/day during [transition period](#Transition_period)) and Dynamics Enterprise gets 20K requests/day (100K requests/day during [transition period](#Transition_period)).

The following Power Automate capabilities are not included:

1. [Robotic process automation](#RPA)
2. [AI builder capacity](#AIbuildercredits)
...
3. What Power Automate capabilities are included in Power Apps licenses?
...
2. Access to [standard connectors](#standardconnectors)
3. Access to [premium connectors](#premiumconnectors) within app context
4. [Business Process Flows](#businessprocessflow) within app context
5. [Custom connectors](#customconnectors) within app context
6. [On premises gateways](#onpremisegateway) within app context
7. Power platform request limits: Power Apps per user gets 5K requests/day (25K requests/day during [transition period](#Transition_period)) and Power Apps per App gets 1K requests/day (10K requests/day during [transition period](#Transition_period)).
...
20. Can I assign licenses to guest users?
...
21. What Power Automate capabilities are included in Windows license?
...
22. Can Windows 10, Windows server 2016 users use PAD?
...
1. What is multiplexing, and is it recommended to use multiplexing to reduce the number of licenses?
...
1. I have a Power App invoked by multiple users and the app in turn calls a flow with premium features. Can I buy a Per flow plan for the flow and enable all users to get premium value?

You can either license all users with premium license or license the flow with a per flow license.

1. I have a flow that is used by my team but runs on a per user license. Is that okay?
...
1. **Automated flow** - User builds a flow that triggers when an item is added to a SharePoint list and updates a SQL db(Premium). Multiple people can add items to the SharePoint list.
...
1. **Instant flow with run only users** - Maker has an Instant flow with premium connectors. Maker shares that flow with his team and allows them to run that flow.

Everyone who invokes the flow need a premium license because it is an instant flow.

1. **Child flows** – A child flow has premium connectors and is invoked by multiple parent flows who do not have premium connectors. Do all parent flows need to be licensed, or is licensing the child flow enough?
...
1. **Automated flow with data verse trigger** – Dataverse triggers allows flows to use invoker user&#39;s connection. Since it is an automated flow, flow only looks for creator/owner&#39;s license

1. **My flow is shared with multiple users who view and edit the flows**. Do Co-owners need premium license to edit the flows?

Co-owners do not need a premium license as the flow only runs in the context of creator or owner/run only user.

1. A premium flow consolidates Azure dev ops items, generates a report, and sends an email to the entire organization. The users of the organization get value from the flow but do not interact with the flow directly.
