...
|**Entitlements**|When [assigned to a cloud flow](/power-automate/desktop-flows/capacity-process#allocate-process-capacity-to-a-cloud-flow), a Power Automate Process license gives the flow [higher action limits](/power-platform/admin/api-request-limits-allocations#request-limits-in-power-automate) and lets it use premium and custom connectors, no matter what license the owning or triggering user has. Flows must be in a solution for a Process license to be assigned.<br><br>When [allocated to a machine](/power-automate/desktop-flows/capacity-process), a Process license becomes an unattended bot that can run one desktop flow at a time. Up to one cloud flow automatically inherits the Process plan from the related machine or machine group that has Process capacity assigned. To run multiple unattended desktop flows in parallel, purchase Process licenses for the maximum number of runs that need to execute in parallel, and allocate Process capacity to the related machines. <br><br> To [trigger attended or unattended desktop flow runs](/power-automate/desktop-flows/trigger-desktop-flows) from cloud flows that are operating under the Process license, the user whose connection is used to run the desktop flow needs a Power Automate Premium license (or other user license with desktop flows entitlement). <br><br>Each Process license includes up to 250,000 actions per day (shown as [Power Platform Requests](/power-platform/admin/api-request-limits-allocations#Request-limits-in-power-automate) in admin center reports). You can stack licenses on one flow, or share one license across a flow group. To learn more, see [Can I assign multiple Process licenses to a single cloud flow?](faqs.md#can-i-assign-multiple-process-licenses-to-a-single-cloud-flow) To size your purchase, estimate daily usage as (actions per run) × (runs per day).|
...
<sup>2</sup> Process licenses can be stacked on one cloud flow, or one license can be shared across a [flow group](/power-automate/flow-groups). Learn more in [Can I assign multiple Process licenses to a single cloud flow?](faqs.md#can-i-assign-multiple-process-licenses-to-a-single-cloud-flow).
