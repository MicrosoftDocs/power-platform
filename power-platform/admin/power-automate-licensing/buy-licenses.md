...
1. To have a flow use the Process license, select  **Edit** in the details panel. You will then see the **Name**, **Description** and **Plan** that the flow uses. If your environment has the Process license assigned, you can move that flow to use the Process license. Otherwise, you need to assign capacity.
...
After you update your flow, anyone will be able to use that flow, even if it leverages premium connectors. Additionally, capacity will is dedicated to that flow.
...
Customers can make a self-service purchase from Power Automate. To do this, they first provide their email address to confirm if they are a user in an existing Microsoft Entra tenant. Next, they need to log in by using their Microsoft Entra credentials. After signing in, they need to indicate the number subscriptions they want to purchase and provide a credit card for payment. When the purchase is complete, they can start using their subscription. The purchaser can also access a limited view of the Microsoft 365 admin center where they can enable other people in their organization to use the product.
