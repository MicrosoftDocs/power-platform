...
In some scenarios, you might not want to use the default sync filter logic. In the image below, an organization is using a sync filter template called **My Outlook Contacts**, but the sync filter criteria in the template doesn't work for the organization.<!--note from editor: Please check the alt text.-->
...
Open XrmToolBox and connect to your organization. On the **Configuration** tab, select **Tool Library**, enter **sync filter** in the search box, and then select **Sync Filter Manager**.<!--note from editor: Suggested.-->
...
5. In the **Edit Filter Criteria** dialog, define the filter criteria for the view. For example, you can set the contact table filter to only synchronize contacts past a certain date. The other conditions that were already included in the existing **My Outlook Contacts** filter remain the same.<!--note from editor: Please check alt text.-->
...
5. On the next screen, when asked to apply the filter<!--note from editor: Edit okay? There seems to be something missing.--> to some users, select **NO**. Note that current users who have been set up with server-side sync already have the **My Outlook Contacts** sync filter, so adding it now would just create an additional contact table filter.  
...
Only the filter templates where the **Is Default** value is set to **True** will be distributed to new users. When new users are set up in your organization, they'll have **My Outlook Contacts Updated** set as the filter criteria along with all of the other default views shown in the preceding image.<!--note from editor: Would a reader with low vision need to know what all these views are? If so, please add complex image markup and list them in the long description.--> This won't change the filter set of any existing user who has already been configured to use server-side sync for appointments, contacts, and tasks. You can delete the old **My Outlook Contacts** filter if you're not going to use it anymore.
...
3. In the pop-up window, search for and select the users who you want to apply the template for, and then select **OK**. You can check the users who have had the filter applied on the **Users Synchronization Filters** tab.<!--note from editor: Edit okay?-->

   After the new filter is applied, a user will be able to see the applied template via **Set Personal Options** settings.<!--note from editor: This doesn't seem to be the right image.-->
...
### How can I check to ensure that the correct list of rows that will be synchronized by a filter are being created in Exchange?<!--note from editor: Edit okay? I didn't understand this.-->
