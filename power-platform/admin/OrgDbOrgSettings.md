...
|-------------------------|-------------------------|-------------------------|
| AutoCreateContactOnPromote | true | Disables the ability to automatically create a contact or lead row for unresolved senders on promote of email<!--note from editor: What does "promote of email" mean? Will the reader know?--> for all the users in the organization. <br> This option can also be disabled from the personal options setting for each user. More information: [Set personal options](/powerapps/user/set-personal-options#email-tab-options) |
...
| ResolveForInactiveEntities | false | When enabled, email addresses for inactive tables are ignored for email address resolution. <br> **Note**: This setting doesn't apply to system users whose accounts have been disabled<!--note from editor: Edit okay? Saying "disabled users who are always excluded" seems like asking for trouble.--> , who are always excluded. |
