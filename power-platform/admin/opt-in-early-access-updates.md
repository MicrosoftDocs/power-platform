...
These settings persist for your environment and you no longer need to update them every release wave. The legacy opt-in (**Manage** and **Update Now**) options are removed.
...
- **Feature visibility** happens on a monthly or semi-annual basis.
  - The **Monthly** release channel means your environment gets new features as soon as the application teams make them available.
  - The **Semi-Annual** channel dates are listed in [General availability deployment](general-availability-deployment.md#deployment-schedule).

> [!NOTE]
> To learn more about the release schedule, see [release schedule and early access](/dynamics365/get-started/release-schedule).

## Prepare for a release wave

The following checklist provides general guidelines to help you prepare for a release wave. These steps are optional.

1. Review the [release plans](/dynamics365/release-plans/).

1. Create a sandbox environment from the production environment.
...
1. Set the Release Channel to **Monthly**. [How to set channel](/power-apps/maker/model-driven-apps/channel-change)

1. Validate that key scenarios work as expected in the sandbox environment after the update finishes.
...
No. Deployments happen every week. The release channel isn't a deployment, but a feature flag that turns on or off depending on your channel setting. To ensure you get the best quality of the new features and capabilities, all customers are required to update to the latest release as scheduled.
