...
### Legacy Opt-In removed
- The legacy opt-in (**Manage** and **Update Now**) options are removed.
- To opt-in Early access features, select **Monthly** release channel.
...
> You might be able to create the copy as an [Early Release environment](early-release.md), putting it first in line for new rollouts.
> This option is also known as _First Release_, _Station 1_, or _Station FRE_.

### Opt-in Steps
1. Go to [PPAC Admin Center](https://admin.powerplatform.microsoft.com).
2. Select **Environments** from the left navigation.
3. Select your environment you want to update.
4. Select **Settings** then **Behavior**.
5. Set your environment's Release channel to **Monthly**.
6. Click **Save**.  **Changes are applied immediately!
7. You can start validation steps at this time. 
...
We recommend you check:
   ...
   - Update internal readiness materials (training and communications) for your organization based on new features or user experiences.
   - If you find any problems during validation, such as regressions or functional or performance problems, contact [Support](https://dynamics.microsoft.com/support/) or get help from [Dynamics 365 forum](https://community.dynamics.com/f).

### Benefits to release channels over legacy opt-in.
- Once the release channel setting is saved, your environment will immediately behave according to the setting chosen.
- Application deployment is no longer required to get the features.
- You can switch between **Monthly** and **Semi-Annual**.
- These settings persist for your environment and you no longer need to update them every release wave.
- More advanced options for preview features.
 
> [!NOTE]
> We recommend only adjusting the Environment's release channel to preview features.
> However you may change release channels for applications, solutions or users.
> For more information about advanced options, see [Changing release channels for model-driven apps](/power-apps/maker/model-driven-apps/channel-change).

### Release Channels Comparison
You can choose to receive features on a **Monthly** basis or a **Semi-annual** basis.
- **Monthly** features become visible every month.  [How to set channel](/power-apps/maker/model-driven-apps/channel-change)
...
Learn more about release channels [Release channels for your model-driven app](/power-apps/maker/model-driven-apps/channel-overview)

# Early access updates FAQ

This article provides answers to frequently asked questions about early access updates and release channels.
