...
1. **(Optional)**: If your organization has reverse proxies configured, enter the IP addresses separated by commas in the **Reverse proxy IP addresses** field. The reverse proxy setting applies to both [IP-based cookie binding](/block-cookie-replay-attack) and the [IP firewall](/ip-firewall). Reach out to your network administrator to get the reverse proxy IP addresses.
