...
|powerPlatform.powerAutomate.disableFlowRunResubmission | Boolean | When this setting is **True**, Power Automate cloud flows initiated using instant triggers cannot utilize the flow run resubmission function to restart a previous flow run.  Default value is **True**.  Set the value to **False** to allow users to resubmit previous flow runs that were started with instant triggers.
