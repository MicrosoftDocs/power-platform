...
Connector action control gives admins the ability to allow/block individual actions within a given connector. Admins can use this capability by clicking on the new “Configure connector” dropdown in the Connectors tab and selecting “Connector actions”.  

image

This opens up a side panel which will allow them to allow/deny specific actions. Admins can also set default value (Allow/Deny) for any new connector actions that will be added to the connector in future. 

image

### Endpoint filtering 

Endpoint Filtering allows admins to govern at a fine grain which specific endpoints will be allowed vs. blocked at a tenant or environment level. This facility is available for HTTP, HTTP with Azure AD, HTTP Webhook, SQL Server, Dataverse (legacy), Azure Blob Storage and SMTP connection endpoints. 
...
Admins can then use this capability by clicking on the new “Configure connector” dropdown and selecting “Connector endpoints”.  

image

This opens up a side panel which will allow them to specify an ordered list of Allow and Deny URL patterns for custom connectors. The last row in the list will always be a rule for ‘*’ which applies to all endpoints in that connector. By default, the * pattern is set up as ‘Allow’ for new DLP policies, but admins can tag this as ‘Allow’/’Deny’. 

image

New rules can be added by clicking on “Add endpoint”. New rules are added to the end of the pattern list (second last to be more specific since ‘*’ will always be the last entry in the list). However, admins can also update the order of the patterns by using the order dropdown or ‘Move up’ and ‘Move down’ buttons. 

image

Once a pattern has been added, admins can edit or delete these patterns by clicking on a specific row and using the ‘Delete’ button. 

## Endpoint input formats and examples 

Each connector has a different notion of what an endpoint means. Further, some endpoints can be defined in multiple formats. Therefore, endpoints have to be entered in all possible formats in order to block makers from using them while creating apps and flows. Admins can either enter the full endpoint name or use pattern matching with ‘*’ when creating an endpoint filtering rule. These rules are entered and represented through an ordered list of endpoint patterns, meaning that they will be evaluated in ascending order of the Order #. Please note that the last rule for any given connector is always “*” Allow/Deny (“Allow” by default, but can be changed to “Deny”). Below is guidance on how to enter connector endpoints while creating rules to allow/deny them. 

SQL Server 

SQL Server connection endpoints have to be listed in <Server_name,database_name> format. A few things to keep in mind: 

Server name can be entered in various formats by makers. Therefore, in order to truly address an endpoint, it has to be entered in all possible formats. For e.g., on-prem instances can be in <machine_name\named_instance,database_name> or <IP address, custom port,database_name> format. In this case, you will have to apply allow/block rules in both formats for an endpoint. E.g., 
...
Dataverse endpoints are represented by environment ID (also known as Org ID). For e.g., contoso.crm.dynamics.com. Please note that only the regular Dataverse connector is currently in scope for endpoint filtering. Dataverse dynamics and Dataverse current connectors are not in scope. Also, local instance of Dataverse (also known as “current environment) can never be blocked for use within an environment. This means that within any given environment, makers can always access the Dataverse current environment instance. Therefore, a rule that says the following.. 

“Allow” “contoso.crm.dynamics.com” 

“Deny” * 

..actually means 

“Allow” “Dataverse current environment” 

“Allow” “contoso.crm.dynamics.com” 

“Deny” * 

In other words, “Allow” “Dataverse current environment” is always implicitly the first rule in the Dataverse endpoint filtering list for any given environment. 

Below are examples of a few scenarios: 

Block Dataverse instances from a specific geography in a specific cloud : 

“Deny” “*.crm2.dynamics.com” 
...
Azure Blob Storage endpoints are represented by Azure storage account name. 
...
‘Allow’ https://management.azure.com/subscriptions* 

‘Deny’ https://management.azure.com/* 
