...
You don't need to add users to environments that have no Dataverse database, because all users in the organization are present in these environments by default. However, to give a user access to an environment's resources, assign a security role to them. For information about assigning a security role to users in an environment, see [Configure user security to resources in an environment](database-security.md).
...
- Automatic user addition to an environment takes time, especially if your organization is large and access to the environment isn't restricted to any security group. As a best practice, restrict access to your environment to a specific set of users by [associating your environment to a security group](control-user-access.md).
...
- Because it can take a long time to automatically add users to an environment, use the following procedure to add specific users to the environment sooner.
...
### Steps to add users to an environment that has a Dataverse database

To add users to an environment that has a Dataverse database, take the following steps:
...
On the **Users** page, you can do the following tasks:

- View the list of users already added to the environment, including those users with enabled and disabled status. For more information, see [Enable or disable users](create-users.md#enable-or-disable-user-accounts).
  
- Search the user list to check if the user you want to add is already present, as automatic user addition might add them. If the user isn't found, select **Add user** in the command bar.

- In the **Add user** pane:
  - Enter the user's name or email address and select it.
  - Select **Add** to add the user to the environment. Ensure you meet the requirements for successfully adding a user. For more information, see [enable a user in an environment](create-users.md#enable-or-disable-user-accounts).
