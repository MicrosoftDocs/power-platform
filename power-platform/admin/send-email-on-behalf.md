...
<!--note from editor: Please note the edit to the title. It and the H1 (and the description too) must be unique in an article.-->
...
-	The user sending the email needs to have read access to the queue. Note that this requirement<!--note from editor: Edit okay?--> only applies to the Send on Behalf permission for the queue. 

## Optional: Send outgoing email as another user with Exchange mailbox delegation

For the email to be shown as "sent on behalf"<!--note from editor: Edit okay?--> of another user in Microsoft Outlook, ensure that the following prerequisites are met: 
...
- The user who is submitting the send request for the outgoing email<!--note from editor: Edit okay? The slash was confusing to me.--> in Dynamics 365 must have at least one of these three permissions:
  - Full Access
