...
> |  **Configure default synchronization method**  |  For any mailbox that's automatically created in model-driven apps in Dynamics 365,  the default email settings defined in this section will be applied when a user or queue is created.<!--Suggested.-->  |  
...
> |  Use tracking tokens  |  Select this check box to use tracking tokens and to configure how model-driven apps in Dynamics 365 display them in the Subject line of email messages.<br /><br /> Tracking tokens provide 100&nbsp;percent tracking accuracy. If you don't want to see tokens in Subject lines, however, consider folder-level tracking, which also provides 100&nbsp;percent tracking accuracy.<br /><br /> You can configure prefixes and other sections of tracking tokens. However, using long prefixes or making too many prefix changes<!--Edit okay? I wondered who or what was making the prefix changes here.--> might cause data to be lost in the message history.<!--Is this what "history" referred to?--> [!INCLUDE[proc_more_information](../includes/proc-more-information.md)] [Email message filtering and correlation](email-message-filtering-correlation.md)  |
...
> |  Maximum file size (in Kilobytes)  | Increase or decrease the maximum file size for attached files. The default size is 5 MB (5,120 KB). The maximum recommended size is 32 MB (32,768 KB). You should not exceed this maximum.<!--Edit okay? Otherwise, you're repeating "maximum recommended... using more isn't recommended."--> |
