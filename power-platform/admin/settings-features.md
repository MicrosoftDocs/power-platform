...
| Allow AI to generate charts to visualize the data in a view (Preview) | When **On**, users can select **Visualize** button on the view pages of model-driven apps to see an AI-generated chart that aggregates the records in the view to show trends and distributions. More information: [Visualize data in a view with Copilot](https://go.microsoft.com/fwlink/?linkid=2300297) | **Auto**<br><br> This feature will be enabled by default for all users that have **Natural language grid and view search (preview)** enabled.|
