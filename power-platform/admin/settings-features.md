...
| Apply AI-generated chart styling to all charts | When **On**, charts created from legacy chart designer will have the same usability enhancements like modern colors and styling as AI-generated charts. Learn more in [Visualize data in a view with Copilot](/power-apps/user/visualize-data-in-copilot). | Auto<br><br>This feature is turned on by default for all users that have **Allow AI to generate charts to visualize the data in a view (Preview)** set to **On** .|
