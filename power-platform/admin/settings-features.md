...
| Setting | Description | Default value |
|---------|-------------|---------------| 
| Disable money saving rules |  When **On**, prevents makers to create money savings rules and track money savings generated by their Power Automate automation. Any pre-existing money saving rule at time of enforcement will stop generating new money saving records but pre-existing money saving records will remain.| Off |
| Time to live (in days) of savings events in Flow Aggregation entity | Determines how long savings records (not savings rules) should be retained in the Flow Aggregation entity. Dataverse automatically deletes records that are older than the specified time-frame. Set the value to -1 to never delete them.| 365 days |

Learn more on [Power Automate savings](/power-automate/savings) for makers.
