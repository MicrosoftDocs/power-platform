...
Allow canvas editors to insert the Copilot answer component, which allows users to receive an AI-powered answer to a predefined data query| When **On**, lets app makers use the Copilot answer control which allows users to receive an AI-powered answer to a predefined questions set by a maker. More information: [Use Copilot answer control for canvas apps (preview)](/power-apps/maker/canvas-apps/copilot-answer-control-overview).| Off |
