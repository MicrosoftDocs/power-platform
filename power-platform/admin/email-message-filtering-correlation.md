...
However, we have introduced a new attribute named 'InternetMessageHeaders' within the Email table to facilitate the identification of auto-generated responses. This attribute is exclusively populated for incoming emails and only if the email itself includes either 'Auto-Submitted' or 'Reply-To' headers. These header values are captured in JSON format.

To utilize the information contained within this 'InternetMessageHeaders' attribute, you can either parse the JSON data to access specific header names and values or simply check if the column contains the header name you're interested in. For instance, if the 'InternetMessageHeaders' column contains 'Auto-Submitted', it signifies that the email was generated automatically.

| Header | Description | Examples
|---------------|----------|----------------|
| Auto-Submitted | If this header is present, it means that the email is an auto-reply. Different email providers may use different values for this header. Additional details can be  found in the [RFC](https://www.rfc-editor.org/rfc/rfc3834).  |Gmail - `Auto-Submitted : auto-replied` </BR></BR>Outlook - `Auto-Submitted : auto-generated` |
| Reply-To | If this header is present, it means the email has a reply-to setup. The value in this header will show the reply-to email address. The format of the value may vary depending on mail providers and other configurations. Additional details can be  found in the [RFC](https://www.rfc-editor.org/rfc/rfc3834#section-3.1.2). |`Reply-To : “Test email <example@contoso.com>”` </BR></BR>`Reply-To : “<example@contoso.com>”` |
