...
The "Regarding" column is present in each email row and is used to link the email to another row. For instance, the email might be linked to an opportunity, order, or case. If you are manually creating or updating an email row, you have the option to use the Regarding column to connect the email to a row from any table that has been set up to permit activities. In case the email is being tracked automatically by Server-Side Synchronization because it was correlated with a previously tracked email, the Regarding column is automatically assigned the same value as the already tracked email.
