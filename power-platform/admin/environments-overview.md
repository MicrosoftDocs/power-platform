...
|Default   | These are a pre-defined type of environment intended for experimentation, exploration and lightweight app trial development. Default environment does not provide any backup guarantees and should not be used for production workloads. Each tenant has a default environment that's created automatically. Its characteristics are discussed in the following section, [The default environment](#default-environment)   |  Limited control. All licensed users<sup>1</sup> have the environment maker role.     |
