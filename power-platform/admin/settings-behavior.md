...
> |**Display behavior**||  
> |Use updated experience for classic settings where possible| Default: Off for existing environment.  If **On**, you will be redirected from the legacy settings to the new experience if that legacy settings has a modern user experience available.|
> | Enable "Switch to classic" from Power Apps| Default: On for existing environment.  If **Off**, you will not see the Switch to classic from Power Apps to use the legacy settings experience.|   
