...
> |--------------|-----------------|  
> |**Refresh channel**||  
> |Model app refresh channel|Default: Semi-annual channel. If **Semi-annual channel**, users in the org will use features from the current release wave which is existing behavior. If **Monthly channel**, users will use features from the current monthly release. This setting can be changed at anytime and users will be switched after one to two browser refreshes. More information: [Manage release channel](/dynamics365/customer-engagement/customize/manage-auto-save)|  
> |**Basic behavior**||  
