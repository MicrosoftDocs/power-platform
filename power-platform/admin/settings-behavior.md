...
> |**Display behavior**||  
> |Use updated experience for classic settings where possible| Default: **Off** for existing environment and **On** for newly created environment after October 2021. If **On**, you will be redirected from the legacy settings to the new experience if that legacy settings has a modern user experience available. More information: [New loction for admin settings](admin-settings.md#new-location-for-admin-settings)|
