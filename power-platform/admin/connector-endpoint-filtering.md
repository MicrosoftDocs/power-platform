...
# Connector endpoint filtering (Public Preview)
...
**Connector endpoint filtering (Public Preview)** allows admins to govern which specific endpoints that makers can connect to when building apps, flows, or chatbots. It is configured within a DLP policy, and it is exclusively available for 6 connectors: HTTP, HTTP with Azure AD, HTTP Webhook, SQL Server, Azure Blob Storage, and SMTP. When a maker tries to connect their app, flow, or chatbot to a blocked endpoint, they will encounter a DLP error message.

## How to add endpoint filtering rules to your DLP policies
