...
- **ApplicationImpersonation role**. You need to create and configure a service account with the **ApplicationImpersonation** role in [!INCLUDE[pn_Exchange](../includes/pn-exchange.md)]. [!INCLUDE[proc_more_information](../includes/proc-more-information.md)] [Impersonation and EWS in Exchange](https://msdn.microsoft.com/library/office/dn722377\(v=exchg.150\).aspx)

- **Secured connection**. The connection between customer engagement apps and Exchange must be encrypted via [TLS/SSL](/windows/win32/secauthn/cipher-suites-in-schannel) and current [cipher suites](server-cipher-tls-requirements.md).

- **Exchange Web Services (EWS)**. Connections to EWS must be allowed through the firewall. Often a reverse proxy is used for the exterior-facing connection. If your EWS endpoint is not exterior-facing, review [IP addresses required](online-requirements.md#ip-addresses-required). Be sure to add IP addresses to the allow list on your on-premises firewall, based on your Dynamics 365 online environment region to allow connections to your Exchange server that is on-premises.
...
1. To set up the **Synchronization methods**, configure the following processing and synchronization options:  
   ...
   - **Email processing for unapproved user and queues**: If at its default setting (enabled), you need to approve emails and queues for user mailboxes as directed in [**Approve email**](#approve-email) later in this topic.  
...
- [**Set mailboxes to the default profile**](#set-mailboxes-to-the-default-profile).
- [**Edit mailboxes to set profile and delivery methods**](#edit-mailboxes-to-set-profile-and-delivery-methods).

### Set mailboxes to the default profile

To set mailboxes to the default profile, take these steps:
...
1. In the command bar, select **Apply Default Email Settings**, verify the settings, then select **OK**.
...
1. Select the mailboxes you want to configure, then select **Edit** in the command bar.
1. Select **General** to set up **Synchronization methods** as follows:
...
1. Expand **Email**, then select **Mailboxes**.
1. In the grid view selector, choose **Active Mailboxes**.
1. Select the mailboxes you want to approve, then select **Approve Email** in the command bar.  
