...
### Email message has "Pending Send" status  
 If you create an email message in Dynamics 365 apps and click the **Send** button, the message will not be sent unless email integration has been correctly configured and enabled for sending email from Dynamics 365 apps.  
...
### Email fails to be sent or received when server-side synchronization is configured with Gmail  
 If Dynamics 365 apps is configured to use Server-Side Synchronization with Gmail, you may encounter one of the following errors:  
...
2. A server error (for example, invalid profile credentials) or a mailbox error (for example, invalid mailbox credentials).  
   ...
   Dynamics 365 apps responds to the error as follows:  
...
### Appointment record is not created in Dynamics 365 apps when tracked by invitee  
 Consider the following scenario regarding tracking an event in Dynamics 365 apps:  
...
5. The invitee logs in to Dynamics 365 apps and navigates to **Marketing** > **Activities** > **Appointment** > **My Appointments**  
  
Result: the appointment is not created in Dynamics 365 apps for the invitee.  
  
This is a known issue and is not supported. If the organizer is someone outside of the Dynamics 365 apps organization, a Dynamics 365 apps user who is an invitee can still track the appointment and have the record created in Dynamics 365 apps.  
   
### Service Appointments and Activities don’t synchronize from Outlook to Dynamics 365 apps 
 Changes made to Service Appointments and Activities in Dynamics 365 apps will update in [!INCLUDE[pn_crm_for_outlook_short](../includes/pn-crm-for-outlook-short.md)] when you synchronize but the reverse is not true. When you make changes to Service Appointments or Activities in [!INCLUDE[pn_crm_for_outlook_short](../includes/pn-crm-for-outlook-short.md)], the changes are not synchronized to Dynamics 365 apps. Service appointments are scheduled by an agent and need free/busy information for resources available only in Dynamics 365 apps.  
