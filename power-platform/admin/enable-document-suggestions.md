...
#### Constructing the external URL
 ...
 You should construct the external URL in a format that the external site understands. For example, for sites that use a construct similar to *`https://contoso.com/search?{0}`*, where **`https://contoso.com/search?`** is the search URL structure and {0} is the keyword string, Document Suggestions passes the keywords in the {0} parameter. The keywords that are passed to the URL come from similar record rules that include entity mappings of **Text Match**. [!INCLUDE[proc_more_information](../includes/proc-more-information.md)] [Use advanced similarity rules to view similar case suggestions](/dynamics365/customer-service/suggest-similar-cases-for-a-case).
