...
If the users are accessing only sign in page, then they won't be counted as anonymous users. There are several types of pages that aren't counted for the anonymous user meter. The pages are described in [Power Pages – Anonymous user meter](pay-as-you-go-meters.md#anonymous-per-user-meter).

### How are analytics providers like Application Insights or Google Analytics counted?

User counts generated by analytics providers like App Insights or Google Analytics can differ from the counts of the Power Pages meters.  
