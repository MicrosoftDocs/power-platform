...
Power Platform [supports CSP](/power-platform/admin/content-security-policy) for model-driven apps.
...
All Microsoft services and products are configured to use the approved cipher suites, in the exact order directed by the Microsoft Crypto Board. For the full list and exact order, see the [Power Platform documentation](/power-platform/admin/server-cipher-tls-requirements).

Information about deprecations of cipher suites is communicated through Power Platform's [Important Changes](/power-platform/important-changes-coming#tls-rsa-cipher-suites-are-deprecated) documentation.
