...
1. Select other settings, as appropriate:
   ...
   - **Reverse proxy IP addresses**: If your organization has reverse proxies configured, enter the IP addresses separated by commas. The reverse proxy setting applies to both [IP-based cookie binding](/block-cookie-replay-attack) and the IP firewall. Reach out to your network administrator to get the reverse proxy IP addresses.
