...
- About nested security groups
  ...
  An example of this scenario: you assigned a security group for the environment when the environment was created. During the lifecycle of the environment, you want to add members to the environment that are managed by security groups. You create a security group in Azure Active Directory—managers, for example—and assigned all your managers to the group. You then add this security group as a child of the environment security group, create a [Dataverse group team](manage-group-teams.md#create-a-group-team), and assign a security role to the group team. Your managers can now access Dataverse immediately.   
...
- When you select a security group, be sure to select an Azure AD security group and not one created in on-premises Windows Active Directory. On-premises Windows AD security groups aren't supported.
