...
no-loc: [ "pac", "pac pages", "pages bootstrap-migrate", "pages clone", "pages download", "pages download-code-site", "pages list", "pages migrate-datamodel", "pages upload", "pages upload-code-site"]
---
<!-- 
...
|[pac pages clone](#pac-pages-clone)|Create Power Pages website content based on existing website content.|
...
Create Power Pages website content based on existing website content.

[!INCLUDE [pages-clone-intro](includes/pages-clone-intro.md)]


### Required Parameters for pages clone

#### `--outputDirectory` `-od`

Path where the Power Pages website content is saved after cloning.
...
### Optional Parameters for pages clone

#### `--name` `-n`

Name for the cloned Power Pages site. If not provided, defaults to 'Copy of <original-site-name>'.

#### `--overwrite` `-o`

Overwrites the existing folder present at the output path.

[!INCLUDE [pages-clone-remarks](includes/pages-clone-remarks.md)]
