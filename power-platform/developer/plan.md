...
Managed Environment is not included as an entitlement in Developer Plan when users run their assets. In other words, users (including makers running their own apps or flows) need a premium license in addition to the developer plan to run their assets in a managed, developer environment.
