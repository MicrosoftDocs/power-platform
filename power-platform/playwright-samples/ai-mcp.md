...
- Gallery items nest multiple locatable elements; the right selector depends on the exact structure
- The iframe boundary (`iframe[name="fullscreen-app-host"]`) must be crossed before any control interaction
- Dataverse-backed views may take 30–60 seconds to populate

With Playwright MCP, the AI assistant can:

1. Navigate to your canvas or model-driven app
2. Inspect control names and ARIA labels on the actual rendered DOM
3. Generate selectors scoped correctly to the iframe
4. Time interactions with appropriate waits based on observed behavior
...
Or use `npx` to run it without global install:
...
> [!NOTE]
> The Playwright MCP server requires Node.js 18 or later and a Chromium-compatible browser installed via `npx playwright install chromium`.
...
Create or update your `CLAUDE.md` or `.claude/settings.json` to include the MCP server. In Claude Code, add the server via the `/mcp` command or by editing `~/.claude/settings.json`:
...
2. Tell your AI assistant to connect to the running browser:

   > "Connect to the browser and navigate to `https://apps.powerapps.com/play/<your-app-id>`"

3. Ask the AI to inspect the app and generate a test:
...
> 2. Filters items to find one titled 'Northwind Traders'
> 3. Clicks that item
> 4. Verifies a details panel appears"

The AI will use the MCP server's `browser_navigate`, `browser_snapshot`, and `browser_click` tools, then emit test code like:
...
> "Navigate to `<model-driven-app-url>`. Inspect the ag-Grid and find the column header for order numbers. Write a test that filters by 'ORD-001', reads the first cell, and verifies the value."
...
The Playwright MCP server exposes the following tools to AI assistants:
...
| Symptom | Resolution |
|---|---|
| AI cannot connect to browser | Ensure `npx @playwright/mcp` is running and your AI client has MCP configured |
| Snapshot shows empty iframe | Navigate into the canvas frame by asking the AI to switch frame context |
| Selectors don't match | App may have reloaded; ask AI to re-snapshot |
